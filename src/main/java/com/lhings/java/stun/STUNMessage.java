...
public class STUNMessage {
	// declare logger
		protected final static Logger log = LhingsLogger.getLogger();
		...
		protected static final byte[] magicCookie = { (byte) 0x21, (byte) 0x12,
				(byte) 0xA4, (byte) 0x42 };
		...
		protected void parseAttributes() {
			...
			while (position < bytes.length) {
				int attrType = ByteMan.bytesToInteger(new byte[] { bytes[position],
						bytes[position + 1] });
				int attrLength = ByteMan.bytesToInteger(new byte[] {
						bytes[position + 2], bytes[position + 3] });
				byte[] attr = Arrays.copyOfRange(bytes, position + 4, position
						+ attrLength + 4);
		...
		public static boolean isWellFormed(byte[] m) {
			...
			// check magic cookie
			if (!(m[4] == magicCookie[0] && m[5] == magicCookie[1]
					&& m[6] == magicCookie[2] && m[7] == magicCookie[3])) {
		...
		public InetSocketAddress getMappedAddress() throws UnknownHostException {
			...
			byte[] port = { (byte) (xorMappedAddress[2] ^ 0x21),
					(byte) (xorMappedAddress[3] ^ 0x12) };
			int portNumber = ByteMan.bytesToInteger(port);
			byte[] ip = { (byte) (xorMappedAddress[4] ^ 0x21),
					(byte) (xorMappedAddress[5] ^ 0x12),
					(byte) (xorMappedAddress[6] ^ 0xA4),
		...
		public String toString() {
			...
			StringBuffer s = new StringBuffer("Message type: "
					+ method
					+ " "
					+ cl
					+ "(0x"
					+ Integer.toHexString(ByteMan.bytesToInteger(new byte[] {
							bytes[0], bytes[1] })) + ")\n");
			s.append("Message length: " + messageLength + " bytes\n");
			s.append("Transaction ID: "
					+ ByteMan.byteArrayToHexString(Arrays.copyOfRange(bytes, 8, 20))
					+ "\n");
			if (this.getMessageClass() == STUNMessage.CL_ERROR) {
				byte[] errorData = this.getAttribute(LyncnatProtocol.attrErrorCode);
				String errorReason = new String(Arrays.copyOfRange(errorData, 4,
						errorData.length));
		...
		public Object[] getErrorCode() {
			if (getMessageClass() == STUNMessage.CL_ERROR) {
				...
				String errorMessage = "";
				try {
					errorMessage = new String(Arrays.copyOfRange(errorAttr, 4,
							errorAttr.length), "utf-8");
				} catch (UnsupportedEncodingException e) {
					log.log(Level.ERROR, "Ooops! Unsupported encoding exception. See stack trace for details.", e);
				}
		...
		public void setMessageType(int cl, int method) {
			int p1 = ((method & 0xf00) << 2) + ((method & 0x080) << 2)
					+ ((method & 0x070) << 1) + (method & 0x00f);
			int p2;
			switch (cl) {
			case STUNMessage.CL_REQUEST:
				p2 = 0x0;
		...
		public void setMessageIntegrity(String username) {
			...
			System.arraycopy(
					ByteMan.integerToBytes(LyncnatProtocol.attrMessageIntegrity),
					0, bytes, position, 2);
		...
		public byte[] getMessageIntegrity() {
			String username = new String(
					attributes.get(LyncnatProtocol.attrUsername),
		...
		public boolean validMessageIntegrity() {
			...
			if (!areEqual)
				log.debug("Message reported MI "
						+ ByteMan.byteArrayToHexString(claimedMI) + " but "
						+ ByteMan.byteArrayToHexString(realMI) + " was found.");
			return areEqual;
		}
		...
		public static int getMethod(int messageType) {
			int r = (messageType & 0x000f) + ((messageType & 0x00e0) >> 1)
					+ ((messageType & 0x0e00) >> 2) + ((messageType & 0x3000) >> 2);
			return r;
		}
